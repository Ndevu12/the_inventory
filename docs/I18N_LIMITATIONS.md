...
| **Catalog / domain content** (product name, category description, supplier name, …) | **Wagtail i18n + [wagtail-localize](https://www.wagtail-localize.org/)** (`TranslatableMixin`, linked rows per `Locale`) | **Database-backed** translatable fields | Wagtail admin / API writes; rows per locale with translation groups |
...
`Reservations`, `Reports`, `CycleCounts`, `BulkOperations`, `Sales`, `Auth`, `Audit`, `SettingsTenant`, `SettingsPlatform`.

### Limitation: other JSON namespaces are unchecked

Keys under **`Common`**, **`Nav`**, **`Inventory`**, **`Procurement`**, **`Errors`**, **`Breadcrumbs`**, **`Shell`**, etc. are **not** in that test. **Wagtail-localize / DB content is not covered** by this test at all.
